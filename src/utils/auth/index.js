...
/** 获取`token` */
export function getToken(roleName) {
 ...
 */
export function setToken(data, roleName) {
  const TokenKey = roleName ? `${roleName}-${TokenKeyBase}` : TokenKeyBase
  ...
  if (data.username && data.roles) {
    ...
    const username = getSessionStorage(userKey)?.username ?? ''
    const roles = getSessionStorage(userKey)?.roles ?? []
    setUserKey(username, roles)
  }
}

/** 删除`token`以及key值为`user-info`的localStorage信息 */
export function removeToken(roleName) {
  const TokenKey = roleName ? `${roleName}-${TokenKeyBase}` : TokenKeyBase
...
/** 格式化token（jwt格式） */
export function formatToken(token, authName) {
